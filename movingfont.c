...
int main(void){
  ...
  while(1){
    ...
    if (ev.type == Expose){
      char *information = "we are testing the moving font";
      XDrawString(dsp, win, gc, 100 , 50,  information, lenstr(information));
    }
	...
	XClearWindow(dsp,win);
	char *information = "we are testing the moving font";
	XDrawString(dsp, win, gc, x ,y,  information, lenstr(information));
	//XDrawString(dsp, win, gc, x , y,  infomin, lenstring(infomin));
