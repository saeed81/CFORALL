...
unsigned char image32[1024*1024*4];

XImage *CreateTrueColorImage(Display *display, Visual *visual, unsigned char *image, int width, int height, int multi)
{
  int i, j;
  //unsigned char *image32=(unsigned char *)malloc(width*height*4);
  //unsigned char *image32=(unsigned char *)malloc(width*height*4);
  printf("inside create true color\n");
  ...
  for(i=0; i<width; i++){
      for(j=0; j<height; j++){
	*p++=(unsigned char)(i *multi ) ; // blue
	*p++=(unsigned char)(multi*(i+j)); // green
	*p++=(unsigned char)(j *multi); // red
	p++;
      }
  }
  printf("end true color\n");
...
int main(int argc, char **argv)
{
  ...
  Window window=XCreateSimpleWindow(display, RootWindow(display, 0), 0, 0, width, height, 1, 0, 0xffffffff);
  ...
  while(1)
    {
      for (int i=0; i <=8;++i ){
	//XClearWindow(display,window);
	ximage=CreateTrueColorImage(display, visual, 0, width, height,i);
	//processEvent(display, window, ximage, width >>i , height>>i);
	XPutImage(display, window, DefaultGC(display, 0), ximage, 0, 0, 0, 0, width, height);
	XFlush(display);
	usleep(1000 *100);
	printf("%d\n",i);
	//XDestroyImage(ximage);
	if (i==0)XClearWindow(display,window);
      }
    }
